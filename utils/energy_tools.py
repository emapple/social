...
def read_eia(year, interval):
    ...
    generic_url = ('https://www.eia.gov/realtime_grid/sixMonthFiles/'
                   + 'EIA930_BALANCE_xxxx_')
    generic_url += 'Jan_Jun.csv' if interval == 1 else 'Jul_Dec.csv'

    df = pd.read_csv(insert_year(generic_url, year),
                     parse_dates=['Data Date'],
                     index_col=['Balancing Authority',
                                'Data Date', 'Hour Number'],
                     usecols=['Balancing Authority', 'Data Date',
...
def read_region(region):
    ...
    reg_dict = {'California': 'CAL', 'Carolinas': 'CAR',
                ...
                'Texas': 'TEX'}
    reg_url = ('https://www.eia.gov/realtime_grid/knownissues/' +
               'xls/Region_xxx.xlsx')
...
def read_all_regions():
    ...
    reg_list = ['California', 'Carolinas',
                ...
                'Southwest', 'Tennessee',
                'Texas']
    df = pd.concat((read_region(region) for region in reg_list),
                   keys=reg_list)
    df = df.sort_index()
